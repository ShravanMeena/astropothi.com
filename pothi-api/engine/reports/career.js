// In-house Career Chart report — deterministic, NO LLM.
//
...
// Contract: generateInhouseCareer(input) → { report, pdfBuffer, kundliData }

import tzlookup from "tz-lookup";

import { buildCalculatedKundliData } from "../astrology/normalize-kundli-data.js";
import { computeCareerFacts } from "../astrology/career-facts.js";
import { buildCareerSections } from "../mapping/career-chapters.js";
...
export async function generateInhouseCareer(input) {
  ...
  const facts = computeCareerFacts(kundliData);
  const P = getLifePack(language);
  const sections = buildCareerSections(facts, P);

  const report = {
    generated_by: "inhouse_career",
    chart_id: 9,
    language,
    kundali_profile: {
