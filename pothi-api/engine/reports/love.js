// In-house Love Chart report — deterministic, NO LLM.
//
...
// placements.
//
// The report is the 24 chapters published for astro_chart_listing id = 1.
// Contract: generateInhouseLove(input) → { report, pdfBuffer, kundliData }
...
import { buildLoveSections } from "../mapping/love-chapters.js";
...
export async function generateInhouseLove(input) {
  ...
  const facts = computeLifeFacts(kundliData, "love");
  const P = getLifePack(language);
  const sections = buildLoveSections(facts, P);

  const report = {
    generated_by: "inhouse_love",
    chart_id: 1,
    language,
    kundali_profile: {
